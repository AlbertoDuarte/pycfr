...
class CounterfactualRegretMinimizer(object):
    def __init__(self, rules):
        ...
        for s in self.profile.strategies:
            ...
            self.counterfactual_regret.append({ infoset: [0,0,0,0,0] for infoset in s.policy })
            self.action_reachprobs.append({ infoset: [0,0,0,0,0] for infoset in s.policy })
    ...
    def cfr_action_node(self, root, reachprobs):
        ...
        if root.play1_action:
            next_reachprobs[root.player] = { hc: action_probs[hc][PLAY1] * reachprobs[root.player][hc] for hc in reachprobs[root.player] }
            action_payoffs[PLAY1] = self.cfr_helper(root.play1_action, next_reachprobs)
        if root.play2_action:
            next_reachprobs[root.player] = { hc: action_probs[hc][PLAY2] * reachprobs[root.player][hc] for hc in reachprobs[root.player] }
            action_payoffs[PLAY2] = self.cfr_helper(root.play1_action, next_reachprobs)
        if root.play3_action:
            next_reachprobs[root.player] = { hc: action_probs[hc][PLAY3] * reachprobs[root.player][hc] for hc in reachprobs[root.player] }
            action_payoffs[PLAY3] = self.cfr_helper(root.play3_action, next_reachprobs)
    ...
    def equal_probs(self, root):
        ...
        if root.play1_action:
            probs[PLAY1] = 1.0 / total_actions
        if root.play2_action:
            probs[PLAY2] = 1.0 / total_actions
        if root.play3_action:
            probs[PLAY3] = 1.0 / total_actions
        if root.raise_action:
...
class ChanceSamplingCFR(CounterfactualRegretMinimizer):
    ...
    def cfr_action_node(self, root, reachprobs):
        ...
        if root.fold_action:
            ...
            action_payoffs[FOLD] = self.cfr_helper(root.fold_action, next_reachprobs)
        if root.play1_action:
            next_reachprobs[root.player] = action_probs[PLAY1] * reachprobs[root.player]
            action_payoffs[PLAY1] = self.cfr_helper(root.play1_action, next_reachprobs)
        if root.play2_action:
            next_reachprobs[root.player] = action_probs[PLAY2] * reachprobs[root.player]
            action_payoffs[PLAY2] = self.cfr_helper(root.play2_action, next_reachprobs)
        if root.play3_action:
            next_reachprobs[root.player] = action_probs[PLAY3] * reachprobs[root.player]
            action_payoffs[PLAY3] = self.cfr_helper(root.play3_action, next_reachprobs)
        if root.raise_action:
